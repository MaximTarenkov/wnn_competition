...
class GRUDualStreamL1FixV2(nn.Module):
    def __init__(
        ...
        # =====================================================================
        # 2. ЕДИНСТВЕННОЕ ИЗМЕНЕНИЕ: delta_proj берет 104 входа вместо 112
        # (исключаем a0..a7 из дифференцирования)
        # =====================================================================
        self.delta_proj = nn.Linear(104, 32, bias=False)
        nn.init.normal_(self.delta_proj.weight, mean=0.0, std=0.01)

        # 192 (база) + 32 (дельта) = 224 (как в hotfix)
        self.gru = nn.GRU(
            input_size=224,
    ...
    def forward(self, x, h=None):
        ...
        # 2. Delta Stream (k=1) по первым 104 колонкам (без a0..a7)
        market_feats = x_flat[:, :104]
        d_proj = self.delta_proj(market_feats).view(B, T, 32)
        d_diff = torch.cat([
